...
function JossyParser(readFile) {
    this._readFile = readFile || function(fname, callback) {
        require('fs').readFile(fname, 'utf8', callback);
    };
    this._cache = {};
}
...
JossyParser.prototype = {
    parseFile: function(fname, callback) {
        ...
        normalizePath(fname, function(err, fname) {
            ...
            that._readFile(fname, function(err, content) {
                if (err) {
                    callback(err);
                    return;
                }
