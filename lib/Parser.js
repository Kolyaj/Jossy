    ...
    parse: function(code, builder) {
        ...
        code.split('\n').forEach((line, i, lines) => {
            if (line.match(/^\s*\/\/#([a-z_]+)(\s+[\s\S]*)?$/)) {
                ...
                var directiveArg = RegExp.$2 ? RegExp.$2.trim() : '';
                var directiveFound = this._directives.some((directive) => {
                    if (directive.name === directiveName) {
                        if (directive.needArgument && !directiveArg) {
                            builder.appendError(`${directiveName} directive expected any argument.`);
                        } else {
                            directive.fn(builder, directiveArg);
                        }
                        return true;
                    }
                    return false;
                ...
                if (!directiveFound) {
                    builder.appendError(`Unknown directive ${directiveName}`);
                }
            } else {
                builder.appendCode(line + (i < lines.length - 1 || needExtraNl ? '\n' : ''));
            }
        });
        builder.end();
        return builder;
